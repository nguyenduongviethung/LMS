...
import { ContentPublicDTO, UpdateContentDTO } from "@shared/src/types/content.types"
...
/* =======================
   1. Hook: form logic
   ======================= */
function useUpdateContentForm(content: ContentPublicDTO, onSuccess: () => void) {
  const { updateContent } = useContent();
  const { addFile, addLink, openFile } = useFile();
  const [formData, setFormData] = useState<UpdateContentDTO>({
    name: content.name,
    description: content.description,
    deadline: content.deadline,
    cutoffScore: content.cutoffScore,
    type: content.type,
    contentFiles: content.contentFiles
  })

  const handleChange =
    <K extends keyof UpdateContentDTO>(key: K) =>
      (value: UpdateContentDTO[K]) => {
  ...
  const handleRemoveFile = async (fileId: number) => {
    setFormData(prev => ({
      ...prev,
      contentFiles: formData.contentFiles.filter(contentFile => contentFile.file.fileId != fileId)
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateContent(content.contentId, formData);
...
function UpdateContentForm({
  ...
  const { formData, handleChange, handleUploadFile, handleAddLink, handleRemoveFile, handleOpenFile, handleSubmit } =
    useUpdateContentForm(content, onSuccess)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle>Cập nhật thông tin nội dung</DialogTitle>
          <DialogDescription>
            Cập nhật thông tin nội dung {content.data.name}.
          </DialogDescription>
        </DialogHeader>
